...
# SAVE VOCABULARY ON FILE
"""    v = set(reuters.words())
    v = [word.lower() for word in v]
    nodup = set(v)
    nodup = [word for word in nodup if word.isalpha()]
    nodup = sorted(nodup)
...
# DEMO CODE
"""with open("reuters_vocabulary.txt", "r") as file:
    vocabulary = [word.strip() for word in file]

query = "Financal Benk"
miss = SpellCorrector(query, vocabulary=vocabulary)
correct = miss.retrive_corrected()
print(correct)"""